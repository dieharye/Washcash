...
export const execute = async (transaction: VersionedTransaction, latestBlockhash: Blockhash, isBuy: boolean = true) => {
  ...
  if (confirmation.value.err) {
    logger.error('Confrimtaion error');
    return '';
  } else {
    if (isBuy) logger.info(`Success in buy transaction: https://solscan.io/tx/${signature}`);
    else logger.info(`Success in Sell transaction: https://solscan.io/tx/${signature}`);
  }
  return signature;
};
